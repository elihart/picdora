class ExtractImgurIds < ActiveRecord::Migration
  def change
    ...
    Image.all.each do |image|
      # Add imgurId
      # Look for a string of at least 4 "word" characters ([a-zA-Z0-9_]) at the end of the url. That should be the id
      # The id is usually after the last slash, but sometimes the url ends with new or all. Take the part of the url
      # after imgur.com, remove common things that get in the way, and then look for the id after the last slash
      pieces = image.url.partition("imgur.com")[2].chomp("/new").chomp("/all").gsub(/\.jpg.*/, "").gsub(/\.gif.*/, "").gsub(/\.png.*/, "").split("/")
      if pieces.empty?
